...
- Multi-session (README recipe 17): each window is a separate process on a port slot claimed by `run_desktop_app.py` **before** `ts_app` is imported (`TS_APP_INSTANCE_SLOT` / `TS_APP_PEER_PORTS` env vars). Never import `ts_app` in the launcher before those are exported. The temp/cache dir is per-slot; the `/_ts_app/current-file` peer endpoint reports process-local state, not the persistent cache — keep it that way or a restarted window falsely claims its old file.
