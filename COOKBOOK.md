# The Time Series App Cookbook

How to build a Dash + Plotly desktop app that lets a user **navigate, inspect,
and annotate long time series** smoothly — zoom, pan, drag-select, auto-pan,
keypress labeling, undo — even when each channel has millions of points.

This repo *is* the reference implementation: a small, runnable, domain-neutral
app. Every recipe points at the exact file here. Nothing in this cookbook
depends on where your signals come from or what they represent — swap in your
own data (one function) and label set (one config file) and the whole
interaction layer is yours.

## Who this is for

- **Agents** building or adapting such an app. Read the [Recipe Index](#recipe-index),
  then pull only the recipes the task needs. Each lists its dependencies and its
  source file.
- **Human designers** who want the *why*. Each recipe has a rationale note,
  because nearly every non-obvious choice here exists to fight one enemy:
  **latency on large signals**.
...
**Reference:** [Cross-cutting patterns](#cross-cutting-patterns) · [Adaptation checklist](#adaptation-checklist) · [Gotcha catalog](#gotcha-catalog)
...
The **recording contract** is the domain-neutral interface (top of `data.py`).
`load_recording` is the *only* function to replace for a real format.
